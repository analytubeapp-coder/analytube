...
const CACHE_TTL = Number(process.env.CACHE_TTL || 60 * 60 * 24);
...
async function getPlan(req: Request): Promise<"Free" | "Starter" | "Pro" | "Business"> {
...
At least 3 exceptions, arrays >=2 items, generated_at must be ISO8601.
...
Enhance this SOP JSON with KPIs, checklists, exceptions, clarity.
...
Validate and repair this SOP JSON to schema compliance.
...
Audit this SOP (ISO/ITIL style). Improve wording slightly.
...
Polish for clarity. Do NOT change structure.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const key = `sop_balanced:${plan}:${hashInput(payload)}`;
    ...
    if (redis) {
      ...
      await redis.set(key, JSON.stringify(out), { ex: CACHE_TTL });
